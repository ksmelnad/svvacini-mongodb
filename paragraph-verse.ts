...
function findClosestNumber(numbers: number[], target: number): number {
...
export async function createParagraphs() {
  ...
  const book = books.find(
    (item) => item.sktextsdata.id === "BV_UPG_MIM_UMIM_AV_MG_BMS"
  );

  try {
    book?.sktextsdata.content.map(async (item) => {
      ...
      if (sectionNumber && Number(sectionNumber) === 0) {
        ...
        const closestSubsectionNumber = findClosestNumber(
          adhikaranasPerPada,
          Number(subSectionNumber)
        );
        console.log(
          "SubsectionId: ",
          `BV_UPG_MIM_UMIM_AV_MG_BMS_${chapterNumber}-${
            Number(sectionNumber) % 4 || 4
          }-${closestSubsectionNumber}`
        );

        const subsectionId = `BV_UPG_MIM_UMIM_AV_MG_BMS_${chapterNumber}-${
          Number(sectionNumber) % 4 || 4
        }-${closestSubsectionNumber}`;
        ...
        if (findSubsection) {
          const paraWrite = await prisma.paragraph.create({
            data: {
              ...
              subsection: {
                connect: {
                  subsectionId: `BV_UPG_MIM_UMIM_AV_MG_BMS_${chapterNumber}-${
                    Number(sectionNumber) % 4 || 4
                  }-${closestSubsectionNumber}`,
                },
              },
...
export async function createVerses() {
  const existingBooks = [
    "BV_SH_SS_PS_RV",
    "BV_GN_LVG",
  ...
  const versedBooks = books.filter((book) =>
    versedBooksArray.includes(book.sktextsdata.id)
  );
...
const getCIDNumber = (line: string) => {
...
export async function createSingleChapterVerses() {
  bgChapter.content.map(async (item) => {
    try {
      const verseWrite = await prisma.verse.create({
        data: {
